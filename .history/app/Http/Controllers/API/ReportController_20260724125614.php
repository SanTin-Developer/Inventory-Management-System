...
class ReportController extends Controller
{
    ...
    public function inventory(Request $request)
    {
        ...
        $products = $query->orderBy('quantity_in_stock')->get();

        $valuation = Product::selectRaw('COALESCE(SUM(quantity_in_stock * cost_price), 0) as total_cost_value, COALESCE(SUM(quantity_in_stock * unit_price), 0) as total_retail_value')
            ->first();
    ...
    public function topProducts(Request $request)
    {
        ...
        $from    = $validated['from'] ?? now()->subDays(30)->toDateString();
        $to      = \Illuminate\Support\Carbon::parse($validated['to'] ?? now()->toDateString())->endOfDay(); // <- ត្រូវបន្ថែម
    ...
    public function slowMovingProducts(Request $request)
    {
        ...
        $products = DB::table('products')
            ->leftJoin('sale_details', function ($join) use ($from, $to) {
                $join->on('sale_details.product_id', '=', 'products.product_id')
                    ->whereExists(function ($query) use ($from, $to) {  // <- ត្រូវប្តូរជា whereExists
                        $query->select(DB::raw(1))
                            ->from('sales')
                            ->whereColumn('sales.sale_id', 'sale_details.sale_id')
                            ->whereBetween('sales.sale_date', [$from, $to]);
                    });
            })
            ->selectRaw('products.product_id, products.product_name, products.product_code, products.quantity_in_stock, COALESCE(SUM(sale_details.quantity), 0) as total_quantity_sold')
            ->groupBy('products.product_id', 'products.product_name', 'products.product_code', 'products.quantity_in_stock')
