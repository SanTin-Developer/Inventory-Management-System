...
class ReportController extends Controller
{
    ...
    public function salesSummary(Request $request)
    {
        ...
        $breakdown = (clone $query)
            ->selectRaw("DATE_FORMAT(sale_date, '{$dateFormat}') as period, COUNT(*) as total_sales, COALESCE(SUM(total_amount), 0) as total_revenue")
            ->groupBy('period')
            ->orderBy('period')
            ->get();
    ...
    public function inventory(Request $request)
    {
        ...
        $query = Product::with('category')
            ->selectRaw('*, (quantiry_in_stock * cost_price) as stock_value');
