...
import { salesApi, productsApi, customersApi } from "../../services/service";
...
const STATUS_STYLE = {
  ...
  Refunded: { color: "#EF4444", bg: "#FEF3F2" },
...
// Total after discount + discount_amount recovers the pre-discount subtotal,
// which lets us derive a discount percentage without an extra API field.
const discountPercentOf = (s) => {
...
export default function Sales() {
  ...
  useEffect(() => {
    ...
    customersApi
      .list({ per_page: 100 })
      .then((res) => setCustomers(extractPaginated(res).items))
      .catch(() => {});
  }, []);
  ...
  const openEdit = async (saleRow) => {
    try {
      const res = await salesApi.get(saleRow.sale_id);
  ...
  const showingFrom = useMemo(
    () => (sales.length ? (meta.current_page - 1) * meta.per_page + 1 : 0),
    [sales, meta],
  );
  const showingTo = useMemo(
    () => (meta.current_page - 1) * meta.per_page + sales.length,
    [sales, meta],
  );
  ...
  return (
    <div className="bg-[#F9FAFB] min-h-full">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 px-6 mb-6">
        {[
          ...
          },
          {
            ...
            value: stats ? stats.cancelled + stats.refunded : undefined,
            icon: XCircle,
            accent: "#EF4444",
          },
        ].map((c) => {
          ...
          <tbody>
            ...
            {!loading &&
              ...
              sales.map((s) => {
                ...
                const discountPercent = discountPercentOf(s);
                return (
                  <tr
                    key={s.sale_id}
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          onClick={() => setViewingId(s.sale_id)}
      ...
      {viewingId && (
        <SaleDetail saleId={viewingId} onClose={() => setViewingId(null)} />
      )}
    </div>
  );
}
