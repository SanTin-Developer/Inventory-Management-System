...
class ProductController extends Controller
{
    ...
    public function store(StoreProductRequest $request)
    {
        ...
        try {
            ...
            $needsGeneratedCode = empty($validated['product_code']);

            // product_code is NOT NULL in the database, so we can't leave
            // it empty on insert and fill it in afterward — the insert
            // itself would fail first. Use a short-lived unique placeholder
    ...
    /**
     * Generate a random "P-XXXXX" code (5 digits, 10000–99999),
     * retrying on the rare chance of a collision with an existing code.
     */
    ...
    public function stats()
    {
        ...
        $byCategory = DB::table('vw_product_inventory')
            ->select('category_name as name', DB::raw('COUNT(*) as category_count'))
